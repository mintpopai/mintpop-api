...
// 说明：complete-registration 直接驱动 CompleteOIDCOAuthRegistration 的核心邀请码/建号逻辑，
// 沿用本文件同组既有用例（TestCompleteOIDCOAuthRegistrationAppliesPendingAdoptionDecision 等）的做法，
// 手工构造 pending session，而非先跑 OIDCOAuthCallback——callback 现在（见 5cab625b）对全新用户
// 一律落"选择页"pending session（LocalFlowState 带 step=choose_account_action_required），
// 命中该 step 时 legacyCompleteRegistrationSessionStatus 会短路直接回显 pending 状态，不会走到
// 邀请码/建号逻辑，无法驱动本次要验证的行为。
func TestCompleteOIDCOAuthRegistrationAllowsEmptyInviteWhenSwitchOff(t *testing.T) {
	...
	session, err := client.PendingAuthSession.Create().
		SetSessionToken("oidc-noinvite-session").
		SetIntent(oauthIntentLogin).
		SetProviderType("oidc").
		SetProviderKey("https://issuer.example.com").
		SetProviderSubject("oidc-noinvite-subject").
		SetResolvedEmail("d3f1a9c2b6e4409a8f6a2b6b6c9a1234@oidc-connect.invalid").
		SetBrowserSessionKey("oidc-noinvite-browser").
		SetUpstreamIdentityClaims(map[string]any{
			"username": "oidc_noinvite",
			"issuer":   "https://issuer.example.com",
		}).
		SetExpiresAt(time.Now().UTC().Add(10 * time.Minute)).
		Save(ctx)
	...
	req.Header.Set("Content-Type", "application/json")
	req.AddCookie(&http.Cookie{Name: oauthPendingSessionCookieName, Value: encodeCookieValue(session.SessionToken)})
	req.AddCookie(&http.Cookie{Name: oauthPendingBrowserCookieName, Value: encodeCookieValue("oidc-noinvite-browser")})
	c.Request = req
	...
	user, err := client.User.Query().Where(dbuser.EmailEQ(session.ResolvedEmail)).Only(ctx)
	require.NoError(t, err)
	require.Equal(t, "oidc", user.SignupSource)
}

// promo_code 优先取请求体，请求体为空才回退 session（pendingOAuthPromoCode）。
func TestCompleteOIDCOAuthRegistrationPromoCodeRequestOverridesSession(t *testing.T) {
	promoRepo := newOAuthPendingFlowPromoRepoStub("REQCODE", 10)
	...
	session, err := client.PendingAuthSession.Create().
		SetSessionToken("oidc-promo-override-session").
		SetIntent(oauthIntentLogin).
		SetProviderType("oidc").
		SetProviderKey("https://issuer.example.com").
		SetProviderSubject("oidc-promo-override-subject").
		SetResolvedEmail("9b1e2c3d4f5a4678b9c0d1e2f3a4b5c6@oidc-connect.invalid").
		SetBrowserSessionKey("oidc-promo-override-browser").
		SetUpstreamIdentityClaims(map[string]any{
			"username": "oidc_promo_override",
			"issuer":   "https://issuer.example.com",
		}).
		SetLocalFlowState(map[string]any{oauthPromoCodeStateKey: "SESSIONCODE"}).
		SetExpiresAt(time.Now().UTC().Add(10 * time.Minute)).
		Save(ctx)
	require.NoError(t, err)

	body := bytes.NewBufferString(`{"promo_code":"REQCODE"}`)
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
	req := httptest.NewRequest(http.MethodPost, "/api/v1/auth/oauth/oidc/complete-registration", body)
	req.Header.Set("Content-Type", "application/json")
	req.AddCookie(&http.Cookie{Name: oauthPendingSessionCookieName, Value: encodeCookieValue(session.SessionToken)})
	req.AddCookie(&http.Cookie{Name: oauthPendingBrowserCookieName, Value: encodeCookieValue("oidc-promo-override-browser")})
	c.Request = req

	handler.CompleteOIDCOAuthRegistration(c)

	require.Equal(t, http.StatusOK, recorder.Code)
	// 请求体显式带 promo_code 时以它为准，session 里的 SESSIONCODE 不应被使用
	require.Equal(t, []string{"REQCODE"}, promoRepo.applyCalls)
}
